...
```
https://www.jma.go.jp/bosai/forecast/data/forecast/{府県予報区コード}.json
```

府県予報区コードは次で調べられます (愛知県 = `230000`)。

```
https://www.jma.go.jp/bosai/common/const/area.json
```

予備として Open-Meteo も残してあります (`source` を `"open-meteo"` にすると
切り替わります)。1時間ごと・降水量つきで粒度は細かいですが、日本の予報精度は
気象庁のほうが信頼できるため既定ではありません。

### 設定

初回実行時に `weather_lamp.json` が作られます。**既定は愛知県なので、違う場合は
`area_code` を書き換えてください。**
...
`area_name` は一次細分区域 (愛知県なら「西部」「東部」)、`temp_point` は気温の
観測地点 (「名古屋」「豊橋」) です。`null` なら先頭が使われます。どちらも
`--raw` で実際の選択肢が確認できます。
...
- **予報文に 雨・雪・雷・みぞれ・ひょう が含まれる**

**朝だけでなく帰宅時刻まで（既定 6時〜21時）を見ます。** 朝が晴れでも夜に降るなら
傘は持って出る必要があるためです。気象庁の降水確率は6時間区切りなので、
時間帯と少しでも重なる区切りを対象にします。

**上着** — 同じ日の**最低気温**が `jacket_temp` 以下なら必要と判断します。
昼が暖かくても朝晩が冷えるなら要るためです。

#### 天気コードの上1桁では降水を判定できません

気象庁の天気コードは 1xx=晴 2xx=くもり 3xx=雨 4xx=雪 ですが、**`202`
「くもり一時雨」は 2 で始まるのに雨を含みます**。そのため予報文の方で判定します。

#### 気温は発表時刻により当日分が無いことがあります

3日予報の `temps` は `timeDefines` と1対1で、00時が最低・09時が最高です。
発表時刻によっては当日分が入らないため、その場合は**週間予報の
`tempsMin`/`tempsMax` で補います**。
...
対象時間帯: 6時〜21時
  ...
  最低気温    : 15.0度
  降水確率最大: 80%
  降りそうな時間: 12-18時, 18-24時

  判定: 傘と上着が必要 -> 紫
    - 降水確率が最大 80% (50% 以上)
    - 予報に雨または雪が含まれる（くもり 昼過ぎ から 雨）
    - 最低気温が 15.0度 (18.0度 以下)
```

`--show` で時間帯ごとの内訳、`--raw` で応答の中身そのものが見られます。
...
できなかったため、公開されているデータ構造に基づいて実装しています。判定ロジック
は固定データで検証済みですが、**実際の応答との突き合わせはラズパイ側で行って
ください。**
...
値が `不明` になる項目があれば、その出力を見れば原因が分かります。
