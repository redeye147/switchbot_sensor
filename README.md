...
**アドバタイズの受信は実機で未確定です。** 制御 (`switchbot_control.py`) は実機で
動作しますが、受信側は手元の個体が**アドバタイズの中身を更新していない**ようで、
検証できていません (下記参照)。

#### アドバタイズが更新されない個体があります

手元の Color Bulb は、色温度を変えても**アドバタイズの中身が一切変化しません**。

```
02 64 01 00 00    ← 全キャプチャで同一
└─ シーケンス番号 = 2 のまま
```

仕様ではシーケンス番号は「更新があると増加する」とされているので、この個体は
状態をアドバタイズに反映していないことになります。`ネットワーク: Wi-Fi 接続中`
のまま変わらないことも、まだクラウドに繋がっていない個体であることを示唆します。

そのため `switchbot_light.py` が表示する値は**古いまま**の可能性があります。
現在の状態を確実に知るには、接続して読む `switchbot_control.py status` を
使ってください。
...
2 バイトのビッグエンディアンとして送る実装にしてあり、**実機 (Color Bulb) で
2700 と 6500 の色の違いを確認済み**です。
