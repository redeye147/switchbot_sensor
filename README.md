...
(`switchbot_protocol.service_device_type()`)。補助として `parse_plug()` は
Wi-Fi RSSI が正の値のペイロードを不正として弾きます (RSSI は 0 以下にしか
ならないため)。

#### `0x6A` は公式一覧にありません

公式の機種一覧 (SwitchBotAPI-BLE の README) に載っているプラグミニは
**`g` (0x67) のみ**です。実機で観測される `j` (0x6A) を JP 版として暫定的に
受け付けていますが、**公式の裏付けはありません**。`--raw` で実データを確認して
ください。
